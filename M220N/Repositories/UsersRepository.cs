...
namespace M220N.Repositories
{
    public class UsersRepository
    {
        ...
        public UsersRepository(IMongoClient mongoClient)
        {
            ...
            _usersCollection = mongoClient.GetDatabase("sample_mflix").GetCollection<User>("users").WithWriteConcern(WriteConcern.WMajority) ;
        ...
        */
        ...
        public async Task<User> GetUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            return await _usersCollection.Find(Builders<User>.Filter.Eq(t => t.Email, email)).FirstOrDefaultAsync(cancellationToken);
        }
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                //var session = await _sessionsCollection.Find(t => t.UserId.Equals(user.Email)).FirstAsync(cancellationToken);

                storedUser.AuthToken = user.AuthToken;

                var x = await _sessionsCollection.UpdateOneAsync(
